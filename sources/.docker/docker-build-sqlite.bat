...
if /I "%NO_CACHE%"=="--no-cache" (
	...
	docker build --progress=plain --build-arg DATABASE_PROVIDER=sqlite --no-cache -f %DOCKER_FILE% -t seats-reservation-management:latest .
	goto :done
)

echo %TAB%- Build image
docker build --progress=plain --build-arg DATABASE_PROVIDER=sqlite -f %DOCKER_FILE% -t seats-reservation-management:latest .
